...
type
  T2DCamera = Object
    ...
    Function  SphereInCamera( P: TVec3f; Radius :Single): boolean;
...
var S,M:TVec3f;
begin
 S := v_sub(P, Vec3f(width*0.5,height*0.5,0));
 M := v_sub(S , Position );
 Position := V_Add(Position, v_mult ( M , FrameTime *0.001));
...
Function  T2DCamera.SphereInCamera( P: TVec3f; Radius:Single ): boolean;
begin
  result :=   (abs(P.x - Position.x -Width  *0.5)< Width  *0.5 + Radius)
          and (abs(P.y - Position.y -Height *0.5)< Height *0.5 + Radius);
end;
...
begin
  ...
  Begin
    ...
    begin
        N := I * 4;
      if Cam.AABBInCamera(Vertex[n],Vertex[n+ 2]) then
      begin
        Move(  Vertex[n],  RVertex[ RCount ], 4* SizeOf(TVec3f));
        Move(TexCoord[n],RTexCoord[ RCount ], 4* SizeOf(TVec2f));
        Move(   Color[n],   RColor[ RCount ], 4* SizeOf(TVec4f));
        inc( RCount , 4);
      end;
    end;
  end;
